...
from alapage.conf import USE_REVERSION, CODE_MODE
if USE_REVERSION:
...
class PageAdmin(MPTTModelAdmin, admin_class):
    ...
    list_display = ['url', 'title', 'published',
                    'registration_required', 'staff_only', 'edited', 'editor']
    ...
    def get_fieldsets(self, request, obj=None):
        ...
        base_fields = (('url', 'parent'), 'title',
                       'template_name', 'published')
        fieldsets = (
            (None, {
                'fields': ('content',)
            }),
    ...
    def change_view(self, request, object_id, form_url='', extra_context=None):
        ...
        if CODE_MODE is False:
            editor = "ckeditor"
        page = Page.objects.filter(pk=object_id).prefetch_related(
            'groups_only', 'users_only')[0]
        # check if the view is restricted so that only the allowed users can
        # edit it
